...
# Create popup tables.
PopNO2Cluster <- popupTable(PopAreaCluster, feature.id = FALSE, row.numbers = FALSE, zcol=c("MSOA Name", "Mean NO2 Conc.", "NO2 Moran's I"))
PopPM2.5Cluster <- popupTable(PopAreaCluster, feature.id = FALSE, row.numbers = FALSE, zcol=c("MSOA Name", "Mean PM2.5 Conc.", "PM2.5 Moran's I"))

# Create palettes.
PalNO2Cluster <- colorBin(PalMi, domain = PopAreaCluster$`NO2 Moran's I`, bins = BrksMi)
PalPM2.5Cluster <- colorBin(PalMi, domain = PopAreaCluster$`PM2.5 Moran's I`, bins = BrksMi)
...
# Create palettes.
PalIMDScoreNO2 <- colorBin("RdBu", domain = PopArea$`IMD NO2 Coef.`, bins = getJenksBreaks(PopArea$`IMD NO2 Coef.`, k=6))
PalIMDScorePM2.5 <- colorBin("RdBu", domain = PopArea$`IMD PM2.5 Coef.`, bins = getJenksBreaks(PopArea$`IMD PM2.5 Coef.`, k=6))
PalRoadDensityNO2 <- colorBin("RdBu", domain = PopArea$`Road Density NO2 Coef.`, bins = getJenksBreaks(PopArea$`Road Density NO2 Coef.`, k=6))
PalRoadDensityPM2.5 <- colorBin("RdBu", domain = PopArea$`Road Density PM2.5 Coef.`, bins = getJenksBreaks(PopArea$`Road Density PM2.5 Coef.`, k=6))
PalDistCLondonNO2 <- colorBin("RdBu", domain = PopArea$`Dist. from C. London NO2 Coef.`, bins = getJenksBreaks(PopArea$`Dist. from C. London NO2 Coef.`, k=6))
PalDistCLondonPM2.5 <- colorBin("RdBu", domain = PopArea$`Dist. from C. London PM2.5 Coef.`, bins = getJenksBreaks(PopArea$`Dist. from C. London PM2.5 Coef.`, k=6))
```
...
# MSOA Area Cluster Results. 
  addPolygons(data = SFAreaCluster, 
              fillColor = ~PalNO2Cluster(`NO2 Moran's I`),
              color = "black",
              stroke = "black", 
              weight = 0.1, 
              opacity = 1.0, 
  ...
  addPolygons(data = SFAreaCluster, 
              fillColor = ~PalPM2.5Cluster(`PM2.5 Moran's I`),
              color = "black",
              stroke = "black", 
              weight = 0.1, 
              opacity = 1.0, 
...
# Legends
  ...
  addLegend(pal = PalNO2Cluster, values = SFAreaCluster$`NO2 Moran's I`, title = "NO2 Moran's I", group = "NO2 Moran's I") %>%
  addLegend(pal = PalPM2.5Cluster, values = SFAreaCluster$`PM2.5 Moran's I`, title = "PM2.5 Moran's I", group = "PM2.5 Moran's I") %>%
...
# Groups, Buttons, etc.
  hideGroup(c("NO2 Concentration","PM2.5 Concentration", "LSOA", "Schools (All)", "GPs (All)", "Hospitals (All)", "Schools (NO2)", "GPs (NO2)", "Hospitals (NO2)", "Schools (PM2.5)", "GPs (PM2.5)", "Hospitals (PM2.5)", "Schools (All) Moran's I", "GPs (All) Moran's I", "Hospitals (All) Moran's I", "Schools (NO2) Moran's I", "GPs (NO2) Moran's I", "Hospitals (NO2) Moran's I", "Schools (PM2.5) Moran's I", "GPs (PM2.5) Moran's I", "Hospitals (PM2.5) Moran's I", "Mean NO2 Conc.", "Mean PM2.5 Conc.", "NO2 Moran's I", "PM2.5 Moran's I", "IMD Score", "Road Density (km/km2)", "Dist. from C. London (km)", "IMD NO2 Coef.", "IMD PM2.5 Coef.", "Road Density NO2 Coef.", "Road Density PM2.5 Coef.", "Dist. from C. London NO2 Coef.", "Dist. from C. London PM2.5 Coef.")) %>% 
